...
f_loudness = 10; %loudness factor for preprozessing

for idxSpeaker=1 :  length(irs.speakerNames)
    tmpIR(idxSpeaker,:,:)=irs.ir{idxSpeaker};
end

if f_loudness > 1/max(max(max(abs(tmpIR))))
    error('f_loudness kleiner wählen')
end
...
for idxSpeaker=1 :  length(irs.speakerNames) %clac for all speakers
    ...
    %WIEDER ENTFERNNEN WENN ES NICHT MEHR GEBRAUCHT WIRD
    %if(1)
    %    figure
    %    plot(0:1/fs:(length(IR_exp)-1)/fs,IR_exp)
    %    IR_exp(:,7)=-IR_exp(:,7);
    %end
    ...
    BRIRLength      = 0.22;                  % Duration of the rendered BRIRs (in seconds)
    ...
    ElOrient        = (-45:ElStep:45)';      % Render BRIRs every ElStep degrees in elevation
    ...
    if(save_BinSim_mat_txt)
        ...
        for iDir = 1:nDirs
            ...
            binsim(struct_it).sourceOrientation = [irs.speakerAzEl{idxSpeaker},0];
        ...
        if idxSpeaker == 1
            save(struct_path,newstructname,'-v7')
        else
            save(struct_path,newstructname,'-append')
        end

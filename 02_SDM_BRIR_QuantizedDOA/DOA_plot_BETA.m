...
for idxSpeaker=1 : length(irs.speakerNames)
    ...
    %RAUMABHÄNGIG----------------------------------------------------------
    MixingTime      = 0.05;                 % Mixing time (in seconds) of the room for rendering. Data after the mixing time will be rendered
    ...
    tmp = DOA(1:100,:);
    %figure
    %scatter3(tmp(:,1),tmp(:,2),tmp(:,3),'filled')
    %xlim([-max(max(abs(tmp))) max(max(abs(tmp)))])
    %ylim([-max(max(abs(tmp))) max(max(abs(tmp)))])
    %zlim([-max(max(abs(tmp))) max(max(abs(tmp)))])

    %DoA und P in csv speichern
    dataCSV = [DOA,P];
    if(0)
        csvName=[irs.room,'_',char(irs.info),'_',irs.speakerNames{idxSpeaker},'_XYZW.csv'];
        writematrix(dataCSV,csvName,"Delimiter",';')
    end
    ...
    switch(plot_mode)
        case "dark"
            cmap = colormap('hot');
            stepw_color = floor(230/(length(ts)));
    ...
    for k = 1:length(ts)-1
        t1 = t_ds+ts(k);
        t2 = t_ds+ts(k+1);
        plot((ts(k):1:ts(k+1))/fs,(abs(P(t1:t2)/max(abs(P)))),'Color',[newcolors(k,:) AlphaColor])
    ...
    for idxk = length(peak_Az_El_R) : -1 : 1
        ...
        if(~isempty(tmp))
            ...
            A =sqrt(tmp(:,3)./((10^(maxdB/10))));
            tmp=tmp(find(A~=0),:);

            for idxC = 1 : length(A)
                Dcount = Dcount + 1;
                newData(Dcount,:)=[idxSpeaker, t(tcount), tmp(idxC,1:2),A(idxC)];
...
writematrix(newData,'test.csv','Delimiter',';')

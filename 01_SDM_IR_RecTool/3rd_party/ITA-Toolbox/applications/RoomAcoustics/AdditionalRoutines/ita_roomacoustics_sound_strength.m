...
sArgs           = struct('pos1_data','itaAudio', 'pos2_reference','anything', 'distanceOfReference', 10, 'freqRange', ita_preferences('freqRange'), 'bandsPerOctave', ita_preferences('bandsPerOctave') , 'linearConvolution', true, 'useSinglePrecision', false, 'waitBar', false);
...
if isa(reference, 'itaAudio')
    ...
    filterSPK = ita_fractional_octavebands(imp,'bandsperoctave',sArgs.bandsPerOctave, 'freqRange', sArgs.freqRange, 'zerophase' ,false);
    ...
    for iFreq = 1:numel(freqVec)
        if any(isnan([idxStart(iFreq) idxEnd(iFreq)]))
            ...
            L_ref(iFreq) = sum(refGefiltertData(idxStart(iFreq):idxEnd(iFreq), iFreq));
        end
    end
    L_ref = L_ref ./ reference.samplingRate;
    
    
elseif isa(reference, 'itaResult')

    
    if ~isequal(reference.freqVector(:), freqVec(:))
        error('frequency vector of reference and freqRange/bandsPerOctave parameter do not match')
    end
    L_ref = reference.freqData;
    ...
    %     end
elseif strcmpi(reference, 'calibratedSource')
    L_ref = ones(numel(freqVec), 1);
else
    error('invalid reference (can be itaAudio, itaResult or ''calibratedSource'')')
end
...
if sArgs.distanceOfReference ~= 10
    % equation A3  inserted in A4 from DIN EN ISO 3382-1:2009-10
    % L_ref_dB  = 10 * log10(L_ref) + 20*log10(10/d)
    %           = 10 * log10(L_ref * (10/d)^2 )     
    %L_ref = L_ref  * sArgs.distanceOfReference / 10; WRONG! %see issue#82
    L_ref = L_ref  * (sArgs.distanceOfReference / 10)^2;

...
function triggerTimeOut = triggerTime(dataMotor)
    ...
    %plot peak locations for debug: findpeaks(abs(dataMotor.timeData),'MinPeakHeight',0.75*max(abs(dataMotor.timeData))
    
    [~,triggerTimeOut] = findpeaks(abs(dataMotor.timeData),dataMotor.samplingRate,'MinPeakDistance',5,'MinPeakHeight',0.75*max(abs(dataMotor.timeData)));

    %on typical measurements: expecting peaks in the first Peak before
    % 10-12 seconds. and last peak towards the end (last 10 s)
    
    if numel(triggerTimeOut) ~= 2
        ita_verbose_info('expecting 2 Trigger detections, please double check the Motor Switch signal')
    end

    if triggerTimeOut(1) > 12
        ita_verbose_info('Expecting 1st Trigger detection, within the first 10 secongs (180s Measurement). Please double check the Motor Switch signal')
    end

    if triggerTimeOut(2) < dataMotor.trackLength-10
        ita_verbose_info('Expecting 2nd Trigger detection, towards end of Measurement. Early trigger detected!! Please check.')
    end
end

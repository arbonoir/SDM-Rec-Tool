classdef  itaHRTF < itaAudio
    ...
    methods %Class related functions       
        ...
        function [HRTFout,idxCoordUnique] = findnearestHRTF(this,varargin)
            if nargin ==2
                coordC = varargin{1};
        ...
        function surf(varargin)
            ...
            switch sArgs.type
                ...
                case 'sphere'
                    surf(this.dirCoord,freqData_dB(idxF,:),'radius',this.dirCoord.r,unused{:});
                    c = colorbar;ylabel(c,'Magnitude in dB')
                case 'phase'
                    phase = unwrap(angle(this.getEar(sArgs.earSide).freqData(idxF,:)));
                    surf(this.dirCoord,phase,'radius',freqData_dB(idxF,:),unused{:});
        ...
        function varargout = ITD(this, varargin)
            ...
            switch sArgs.method
                case 'phase_delay'
                    ...
                    addDelay = 0.005; %the additional 5ms help stabilize the calculations
                    if this.trackLength < 0.1 %2*0.005
                        addDelay = 0.001; %unwrap gets unstable if IR is close to center of Dataframe -> reduce addDelay
                    end
                    thisC = ita_time_shift(this,tau(idxMin)+addDelay,'time'); 

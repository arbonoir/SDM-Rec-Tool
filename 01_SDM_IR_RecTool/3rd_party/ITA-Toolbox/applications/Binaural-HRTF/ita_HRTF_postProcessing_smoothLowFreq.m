...
%                                       with unwrap phase
%           'extrapolationValue' (1)       : Value at 0 Hz to extrapolate data
%           towards (magnitude and phase) below cutOffFrequency (one value per channel)
%
%  Example:
...
sArgs        = struct('pos1_data','itaAudio', 'cutOffFrequency', 100,'upperFrequency',300,'timeShift',0,'extrapolationValue',1);
...
for instIndex = 1:length(input)
  ...
  interpValues1 = interp1([0 tmp.freqVector(binIdxAtLower:binIdxAtUpper)], [ones(1,tmp.nChannels).*abs(sArgs.extrapolationValue); abs(dataFromLowerToUpper)], tmp.freqVector(1:binIdxAtUpper));
  % why dont we just use spline interpolation towards a fixed value at zero
  % Hz taking care of magnitude and phase?? (HBR)
